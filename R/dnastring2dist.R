...
#' @name dnastring2dist
#' @description This function calculates pairwise distances for all combinations of a \code{DNAStringSet}.
#' @param dna \code{DNAStringSet} [mandatory]
#' @param model specify model either "IUPAC" or any model from \code{ape::dist.dna} [default: IUPAC]
#' @param threads number of parallel threads [default: 1]
#' @param score \code{scoringMatrix} use scoring matrix to calculate distances [default: NULL]
#' @param ... other \code{ape::dist.dna} parameters (see \code{\link[ape]{dist.dna}})
...
dnastring2dist <- function(dna,
                           ...
                           score = NULL,
                           ...){
  if(class(dna) != "DNAStringSet"){stop("Error: Input needs to be DNAStringSet")}
  if(is.null(score) & !model %in% c("IUPAC", "raw", "N", "TS", "TV", "JC69", "K80", "F81", "K81",
                   "F84", "BH87", "T92", "TN93", "GG95", "logdet", "paralin",
                   "indel", "indelblock")){stop("Error: either choose model 'IUPAC' or '?ape::dist.dna'")}
  if(!is.null(score)){
    OUT <- rcpp_distSTRING(dnavector = as.character(dna), scoreMatrix = score, ncores = threads)
    OUT$distSTRING <- as.data.frame(OUT$distSTRING)
    OUT$sitesUsed <- as.data.frame(OUT$sitesUsed)
  }
  if(is.null(score) & model == "IUPAC"){
    OUT <- rcpp_distSTRING(dnavector = as.character(dna), scoreMatrix = iupacMatrix(), ncores = threads)
    OUT$distSTRING <- as.data.frame(OUT$distSTRING)
    OUT$sitesUsed <- as.data.frame(OUT$sitesUsed)
  }
  if(is.null(score) & model != "IUPAC"){
    distSTRING_ <- as.matrix(ape::dist.dna(x = dnastring2dnabin(dna), model = model, ...))
    sitesUsed_ <- rcpp_pairwiseDeletionDNA(dnavector = as.character(dna), ncores = threads)
